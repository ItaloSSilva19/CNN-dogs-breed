...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print(f'% Match: {results_stats_dic["pct_correct_breed"]}')
    
    # Dog misclassifications    
    if print_incorrect_dogs and (results_stats_dic["n_dogs_img"] + results_stats_dic["n_notdogs_img"]) != results_stats_dic["n_images"]:
        print('\nDogs misclassifieds:\m')
        for key, value in results_dic.items():
            if value[3] == 1 and value[3] != value[4]:
                print(f'Pet filename: {key}\nInfo:{value}\n')
    else:
        print('There is no label misclassified as dogs')
    
    # Breed misclassifications
    if print_incorrect_breed and results_stats_dic["pct_correct_dogs"] != results_stats_dic["pct_correct_breed"]:
        print('\nBreeds misclassifieds:\n')
        for key, value in results_dic.items():
            if sum(value[3:]) == 2 and value[2] == 0:
                print(f'Pet filename: {key}\nInfo:{value}\n')
    else:
        print('There is no breed misclassified')
